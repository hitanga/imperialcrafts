 ...
 */
 	...
 	return{
 		  ...
 		  initAfterDocumentReady:function(){
    		...
    		$(window).load(function(){
				...
				try{
					viewSwitcher.init('#products', '#view_switcher', 'grid');
				} catch(error){
					if(window.console){
						console.log(error);
					}
				...
				try{
					...
					$('.cat-item, .page_item').hover(function() {
					  ...
					  $(this).find('>.children').slideToggle('fast', function() {
					    // Animation complete.
					  });
					});
 ...
 function ListingViewSwitcher(){
 	...
 	(function ($) {
	    $.fn.AutoHeight = function () {
	        ...
	        return this.css("height", reset).each(function () {
 			...
 			$(this).find('.action').css('bottom', '20px');
 	...
 	return{
 		init:function(container, switcher, defaultState){
 			...
 			$(productsContainer).removeClass('grid');
 			$(productsContainer).removeClass('list');
 			$(productsContainer).addClass(defaultState);
 		}
 	}
